...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for possible_action in self.all_actions:
            nodeA = PgNode_a(possible_action)
            # iff all prerequisite literals for the action hold in Si
            if nodeA.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(nodeA) # it's a subset then add it to that level
                # connected to the S node instances in the appropriate s_level set
                for nodeS in self.s_levels[level]:
                    if nodeS in nodeA.prenodes:
    ...
    def add_literal_level(self, level):
        ...
        for nodeA in self.a_levels[level-1]: # previous A level
            for nodeS in nodeA.effnodes: # iterate through each effnodes of nodeA
                if nodeS.symbol:         # A literal only can result with a positive affect
                    self.s_levels[level].add(nodeS)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        result =  set(node_a1.action.effect_add) & set(node_a2.action.effect_rem) | set(node_a1.action.effect_rem) & set(node_a2.action.effect_add)
        return bool(result)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Step by step calculate the results of effect_add, precond_neg & effect_rem, precond_pos of actions 1 & 2
        result_eff_add_pre_neg = set(action1.effect_add) & set(action2.precond_neg) | set(action2.effect_add) & set(action1.precond_neg)
        result_eff_rem_pre_pos = set(action1.effect_rem) & set(action2.precond_pos) | set(action2.effect_rem) & set(action1.precond_pos)
        return bool(result_eff_add_pre_neg | result_eff_rem_pre_pos)
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # after a rigorous discussion on the forum thanx to chris_lapollo for clarifing on the implementation direction
        for parent1 in node_a1.parents:
            for parent2 in node_a2.parents:
                if parent1 != parent2 and (parent1.is_mutex(parent2) or parent2.is_mutex(parent1)):
                    return True
        return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        isSymbolSame = node_s1.symbol == node_s2.symbol
        isSignDifferent = node_s1.is_pos != node_s2.is_pos # test for negation
        return isSymbolSame and isSignDifferent
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for parent1 in node_s1.parents:
            for parent2 in node_s2.parents:
                if not ((parent1.is_mutex(parent2) or parent2.is_mutex(parent1)) and node_s2 not in parent1.children and node_s1 not in parent2.children): # struggled initially with the not part but with the help of Forum mentor was able to successfully resolve the issue
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for eachGoal in self.problem.goal:
            found = False
            for level in range(len(self.s_levels)):
                for eachState in self.s_levels[level]:
                    symbol = expr(eachState.symbol)
                    if not eachState.is_pos:
                        symbol = '~' + symbol     # negate it
                    if symbol == eachGoal:
